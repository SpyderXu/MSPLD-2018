...
mean_image                  = fullfile(pwd, 'models', 'pre_trained_models', 'mean_image.mat');
...
for idx = 1:numel(models)
            mAPs{idx} = weakly_co_test_mAP(conf, dataset.imdb_test, dataset.roidb_test, ...
...
mAPs{3}                     = weakly_co_test_mAP(conf, dataset.imdb_test, dataset.roidb_test, ...
                                'net_defs',         net_defs, ...
...
for index = 1:size(rfcn_model, 2)
  ...
  for idx = 1:numel(models)
    S_mAPs(idx, index) = weakly_co_test_mAP(conf, dataset.imdb_test, dataset.roidb_test, ...
                             'net_defs',         {models{idx}.test_net_def_file}, ...
  ...
  S_mAPs(end, index)   = weakly_co_test_mAP(conf, dataset.imdb_test, dataset.roidb_test, ...
