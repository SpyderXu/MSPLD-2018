function save_model_path = weakly_train_v2(conf, imdb_train, roidb_train, varargin)
% --------------------------------------------------------
...
%% making tran/val data
    ...
    %% Show Box Per class
    boxes_per_class   = zeros(numel(conf.classes), 2);
    for index = 1:numel(image_roidb_train)
...
%% training
    ...
    previous_model = weakly_supervised(warmup_roidb_train, opts.solver_def_file, opts.net_file, opts.val_interval, opts.snapshot_interval, ...
		opts.box_param, conf, cache_dir, 'Epoch0', model_suffix, 'final', opts.step_epoch, opts.max_epoch);

    for index = 1:numel(conf.base_select)
        loop = conf.base_select(index);
		caffe.reset_all();
		caffe_test_net = caffe.Net(opts.test_def_file, 'test');
		caffe_test_net.copy_from(previous_model);

        fprintf('\nStart Loop %2d Epoch\n', loop);
		mx_select             = conf.mx_select * (1+loop) * loop / 2;
		select_per_class      = int32(boxes_per_class / min(boxes_per_class) * mx_select);
		assert (all(select_per_class > 0));

		debug_dir             = ['epoch_', num2str(loop)];
		new_image_roidb_train = weakly_get_fake_gt_v2(conf, caffe_test_net, image_roidb_train, opts.box_param.bbox_means, opts.box_param.bbox_stds, select_per_class, debug_dir);
		previous_model        = weakly_supervised([warmup_roidb_train;new_image_roidb_train], opts.solver_def_file, opts.net_file, opts.val_interval, opts.snapshot_interval, ...
									opts.box_param, conf, cache_dir, ['Epoch', num2str(loop)], model_suffix, 'final', opts.step_epoch, opts.max_epoch);
    end
